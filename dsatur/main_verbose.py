...
def GCP(G):
    ...
    while (-1 in colors):
        ...
        #nbcolored_nodes = sum([1 for i in colors if i !=-1])
        #Choose node
        ...
        if sat.count(max(sat))==1 :
           ...
           print("chosen from sat degree",max_sat);
        else :
           print("max(d)", max(d), "d", d)
           chosen_node = d.index(max(d))
           print("chosen from degree");
        #chosen_node = sat.index(max(sat)) if sat.count(max(sat))==1 else d.index(max(d))
        # initilize possible_colors for this node
        possible_colors = [i for i in range(nbcolors)]
        print("Node :", chosen_node , "possible_colors", possible_colors)
        ...
        print("colors",colors)
        print("voisins",voisins(G, chosen_node))
        ...
        #Test => it choses again treated nodes that have degree == -1 because of their saturation ?
